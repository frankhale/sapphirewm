/*
 * image.cc - Defines how images are handled.
...
XImage *BImage::renderXImage(void) {
  ...
  if (control->dither()) {
    short er, eg, eb, *ored, *og, *ob, *nor, *nog, *nob, *por, *pog, *pob;
    unsigned short *ort, *ogt, *obt;
    
    control->getDitherBuffers(width + 2, &ored, &og, &ob, &nor, &nog, &nob,
			      &ort, &ogt, &obt);
    if ((! ored) || (! og) || (! ob) || (! nor) || (! nog) || (! nob) ||
	(! ort) || (! ogt) || (! obt)) {
      fprintf(stderr,
	...
	*(nob + x) = *(blue + i);
      ...
      for (x = 0; x < width; x++) {
	if (*(ored + x) > 255) *(ored + x) = 255;
	else if (*(ored + x) < 0) *(ored + x) = 0;
...
BImageControl::BImageControl(Display *dpy, int depth, Window root, int screen,
	Visual *vis) {
  //display = wm->getDisplay();
  //screen_depth = wm->getDepth();
  //window = wm->getRootWindow();
  //screen_number = wm->getScreen();
  ...
  case TrueColor:
    {
      ...
      unsigned long mask = visual->red_mask, emask = mask;
  ...
  case StaticColor:
    {
      //colors_per_channel = wm->getColorsPerChannel();
...
BImageControl::~BImageControl(void) {
  ...
  if (cache->count()) {
    //int i, n = cache->count();
    //printf("BImageContol::~BImageControl: pixmap cache - "
	//    "releasing %d pixmaps\n", n);
	...
	for (; it.current(); it++) {
	  XFreePixmap(display, it.current()->pixmap);
	}
...
Pixmap BImageControl::searchCache(
				  //char* name,
				  unsigned int width, 
				  unsigned int height,
...
void BImageControl::removeAllImages() {
	...
	LinkedListIterator<Cache> it(cache);
	for (; it.current(); it++) XFreePixmap(display, it.current()->pixmap);
